...
std::vector<std::uint8_t> readFrame(ISerialPort& port,
    ...
    // 1) Read the 5-byte preamble: header(2) + totalLen(3).
    std::uint8_t preamble[5] = {0};
    readExact(port, preamble, 5, deadline);

    if (preamble[0] != proto::kFrameHeader0 || preamble[1] != proto::kRespHeader1) {
        throw ProtocolError("bad response frame preamble");
    }
